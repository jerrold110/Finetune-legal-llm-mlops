...
def get_invocation_params():
    ...
    parameters = {'temperature': 0.2,
                ...
                'json_schema': {'name': 'hypothesis_list',
                'schema': {'type': 'object',
                    ...
                    'properties': {'hypothesis': {'type': 'string'},
                        ...
                        'label': {'type': 'string',
                        'enum': ['entailment', 'contradiction', 'not_mentioned']},
...
def get_quote_drift_score(scorer, quote, contract):
    """
    This ensures that the quote is from the contract and not a hallucination. Checks in the form of precision score
    """
    scores = scorer.score(target=contract,
                          prediction=quote)
    rouge2_precision = scores['rouge2'].precision

    return rouge2_precision

def get_payload(sys_prompt, input_contract, parameters):
    chatml_prompt = (
        "<|im_start|>system\n"
        f"{sys_prompt}<|im_end|>\n"
        "<|im_start|>user\n"
...
def invoke_endpoint_with_ica(sm_client, payload, endpoint_name, adapter_name) -> list[dict]:
    """"""
...
def request_handler(input_contract):
    ...
    sm_client = boto3.client('sagemaker-runtime', config=custom_config, region_name='us-east-1')

    # Get prompt template
    start = time.perf_counter()

    key = '/'.join(prompt_template_uri.split('/')[3:])
    response = s3_client.get_object(Bucket="legal-llama-data", Key=key)
